...
label new_day(time_of_new_day = None, is_check_event=True):
    if(not get_flags("not_can_spend_time")):
        ...
        call after_spending_time(is_check_event = is_check_event) from _call_after_spending_time_2
    else:
...
label wait(wait_hour=DEFAULT_WAIT_HOUR, is_check_event=True):
    if(not get_flags(flag_id = "not_can_spend_time")):
        if(tm.new_hour(wait_hour)):
            if (not map_open):
                call after_spending_time(is_check_event = is_check_event) from _call_after_spending_time_3
        else:
...
label hour_change(new_hour, is_check_event=True):
    if(not get_flags(flag_id = "not_can_spend_time")):
        $ tm.hour = 9
        call after_spending_time(is_check_event = is_check_event) from _call_after_spending_time_6
    else:
        "[DEFAULT_BLOCK_SPENDTIME_DIALOGUE]"
    return
...
label after_spending_time(is_check_event=False, is_check_routines=True):
    call update_current_flags from _call_update_current_flags_1
    ...
    call set_background_nqtr from _call_set_background_nqtr_1
    return
